...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    let firstItem: { boxId: string; categoria: string; talla: string; exclusiones: string; tipo?: string; mensajeRegalo?: string; estampado?: boolean; nombreEstampado?: string; numeroEstampado?: string; jerseySlots?: { talla: string; tipo: string }[] } = { boxId: meta.boxId ?? '', categoria: meta.categoria ?? '', talla: meta.talla ?? '', exclusiones: meta.exclusiones ?? '' }
    if (meta.items) {
      try {
    ...
    try {
      await sendOrderConfirmationEmail({
        ...
        nombreEstampado: firstItem.nombreEstampado || null,
        numeroEstampado: firstItem.numeroEstampado || null,
        jerseySlots: firstItem.jerseySlots ?? null,
        amountTotal: session.amount_total ?? 0,
        amountSubtotal,
        amountDiscount: amountDiscount || null,
        promoCode,
