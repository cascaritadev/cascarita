...
const CATEGORIA_TO_TIPO: Record<string, string> = {
  Clubes: 'clubes', Selecciones: 'selecciones', Retro: 'retro',
}
...
function ConfiguradorContent() {
  ...
  const [presetTalla, setPresetTalla] = useState('M')

  // jerseySlots se usa para TODOS los modos multi (preset y mix)
  const [jerseySlots, setJerseySlots] = useState<JerseySlot[]>(
    Array.from({ length: boxInfo.jerseys }, () => ({
      talla:           'M',
      tipo:            initialTipo === 'retro' ? 'retro' : initialTipo === 'selecciones' ? 'selecciones' : 'clubes',
      estampado:       false,
      nombreEstampado: '',
      numeroEstampado: '',
    }))
  ...
  function changeBoxMode(mode: BoxMode) {
    setBoxMode(mode)
    if (mode !== 'mix') {
      // Preset: sincronizar tipo de todos los slots
      setJerseySlots(prev => prev.map(s => ({ ...s, tipo: mode })))
    }
  }

  function changePresetTalla(size: string) {
    setPresetTalla(size)
    // Actualizar talla en todos los slots del preset
    setJerseySlots(prev => prev.map(s => ({ ...s, talla: size })))
  }
  ...
  function confirmSlot() {
    ...
    if (t && !exclusiones.includes(t)) setExclusiones([...exclusiones, t])
    setEditingSlot(null); setEditingValue('')
  }
  function removeExclusion(eq: string) { setExclusiones(exclusiones.filter(e => e !== eq)) }
  ...
  if (!isMultiJersey) {
    // Debutante: single top-level estampado
    computedPrice = getBoxPrice(boxId, selectedTipo) + (estampado ? ESTAMPADO_PRICE : 0)
  } else {
    // Multi (preset o mix): siempre usa jerseySlots
  ...
  function handleAgregarAlCarrito() {
    if (!isMultiJersey) {
      addItem({
        boxId, boxName: `Mystery Box ${boxInfo.name}`, categoria,
        talla: selectedSize, tipo: selectedTipo,
        exclusiones,
        ...
        price: computedPrice, priceDisplay: computedPriceDisplay, jerseys: boxInfo.jerseys,
      })
    } else {
      // Multi (preset o mix): siempre pasa jerseySlots
      const slots = jerseySlots.map(s => ({
        talla: s.talla,
        tipo:  s.tipo,
    ...
    <div className="bg-surface text-on-surface">
      ...
      <main className="max-w-screen-2xl mx-auto px-6 py-12 lg:py-20 flex flex-col lg:flex-row gap-12">
        <section className="flex-grow space-y-16">
          <header>
            <p className="text-xs font-bold tracking-[0.2em] text-primary uppercase mb-2 font-label">Personalización</p>
            <h1 className="font-headline font-black text-4xl md:text-6xl text-on-surface tracking-tighter uppercase leading-none">
              Personaliza tu <br /><span className="text-gradient-primary">Experiencia</span>
            </h1>
          </header>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {SIZES.map(size => (
                  <button key={size} onClick={() => setSelectedSize(size)}
                    className={`border-2 p-6 rounded-lg transition-all flex flex-col items-center gap-2 ${selectedSize === size ? 'border-primary bg-primary-container text-white' : 'border-outline-variant hover:border-primary'}`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {TIPOS_SINGLE.map(tipo => (
                  <button key={tipo.id} onClick={() => setSelectedTipo(tipo.id)}
                    className={`border-2 p-6 rounded-xl transition-all text-left flex flex-col gap-3 ${selectedTipo === tipo.id ? 'border-primary bg-emerald-50 shadow-md' : 'border-outline-variant hover:border-primary'}`}>
                    <span className={`material-symbols-outlined text-3xl ${selectedTipo === tipo.id ? 'text-primary' : 'text-zinc-400'}`}>{tipo.icon}</span>
                    <div>
                    ...
                    {selectedTipo === tipo.id && <span className="text-[10px] font-black uppercase tracking-widest text-primary">Seleccionado ✓</span>}
                  </button>
                ))}
              </div>
            </div>
          ...
          {isMultiJersey && (<>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {BOX_MODES.map(opt => {
                  const a = ACCENT[opt.accent]; const active = boxMode === opt.id
                  return (
                    <button key={opt.id} onClick={() => changeBoxMode(opt.id)}
                      className={`flex flex-col gap-3 p-5 rounded-xl border-2 text-left transition-all ${active ? `${a.border} ${a.bg} shadow-md` : 'border-outline-variant hover:border-primary'}`}>
                      <span className={`material-symbols-outlined text-3xl ${active ? a.icon : 'text-zinc-400'}`}>{opt.icon}</span>
                      <div>
                      ...
                      {active && <span className={`text-[10px] font-black uppercase tracking-widest ${a.badge}`}>Seleccionado ✓</span>}
                    </button>
                  )
                })}
              </div>
            ...
            {/* Step 02 — Preset: una sola talla */}
            {isPresetMode && (
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <span className="font-headline font-black text-2xl text-primary/20">02</span>
                  <h2 className="font-headline font-bold text-xl uppercase tracking-tight">
                    Talla <span className="text-sm font-medium text-on-surface-variant normal-case tracking-normal">(todos los jerseys)</span>
                  </h2>
                </div>
                {boxMode === 'retro' && (
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                  {SIZES.map(size => (
                    <button key={size} onClick={() => changePresetTalla(size)}
                      className={`border-2 p-4 rounded-lg transition-all flex flex-col items-center gap-1 ${
                        presetTalla === size
                          ? boxMode === 'retro' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-primary bg-primary-container text-white'
                  ...
                  {jerseySlots.map((slot, index) => (
                    <div key={index} className="bg-surface-container rounded-xl p-6 space-y-4">
                      <div className="flex items-center gap-3">
                        <span className="w-7 h-7 rounded-full bg-primary text-white text-xs font-black flex items-center justify-center shrink-0">{index + 1}</span>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {TIPOS_MIX.map(tipo => (
                            <button key={tipo.id} onClick={() => updateSlot(index, 'tipo', tipo.id)}
                              className={`flex flex-col items-center gap-1.5 px-3 py-3 border-2 rounded-xl transition-all ${
                                slot.tipo === tipo.id
                                  ? tipo.retro ? 'border-amber-500 bg-amber-50' : 'border-primary bg-emerald-50'
                                  : 'border-outline-variant hover:border-primary'
                              }`}>
                              <span className={`material-symbols-outlined text-xl ${slot.tipo === tipo.id ? (tipo.retro ? 'text-amber-500' : 'text-primary') : 'text-zinc-400'}`}>{tipo.icon}</span>
                              <span className={`text-[10px] font-black uppercase tracking-tight text-center leading-tight ${slot.tipo === tipo.id ? (tipo.retro ? 'text-amber-600' : 'text-primary') : 'text-zinc-500'}`}>{tipo.label}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {SIZES.map(size => (
                            <button key={size} onClick={() => updateSlot(index, 'talla', size)}
                              className={`px-3 py-2 border-2 rounded-lg text-xs font-black uppercase transition-all ${slot.talla === size ? 'border-primary bg-primary-container text-white' : 'border-outline-variant hover:border-primary'}`}>
                              {size}
                            </button>
                          ))}
                        </div>
                      ...
                      {/* Estampado */}
                      <button onClick={() => updateSlot(index, 'estampado', !slot.estampado)}
                        className={`w-full flex items-center justify-between gap-3 px-4 py-3 border-2 rounded-xl transition-all text-left ${slot.estampado ? 'border-primary bg-emerald-50' : 'border-outline-variant hover:border-primary'}`}>
                        <div className="flex items-center gap-3">
                      ...
                      {slot.estampado && (
                        <div className="grid grid-cols-2 gap-3 bg-white rounded-lg p-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <input type="text" maxLength={2} value={slot.numeroEstampado}
                  ...
                  if (isEditing) return (
                    <div key={i} className="border-2 border-primary bg-white rounded-xl min-h-[52px] px-3 flex items-center">
                      <input autoFocus
                        ...
                        placeholder="Equipo..." value={editingValue}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-surface-container p-8 rounded-xl space-y-4">
              ...
              <div className="relative">
                <textarea maxLength={150} rows={3} value={mensajeRegalo} onChange={e => setMensajeRegalo(e.target.value)}
          ...
          {/* Extra: Estampado ─────────────────────────────
              Debutante: toggle único
              Preset multi: toggle por jersey (+$200 c/u)
              Mix: ya está en las fichas de arriba, no se repite
          ─────────────────────────────────────────────── */}
          {!isMixMode && (
            <div className="space-y-4">
              ...
              {/* Debutante: toggle único */}
              {!isMultiJersey && (<>
                <button onClick={() => setEstampado(p => !p)}
                  className={`w-full flex items-center justify-between gap-4 p-6 rounded-xl border-2 transition-all text-left ${estampado ? 'border-primary bg-emerald-50 shadow-md' : 'border-outline-variant hover:border-primary'}`}>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant font-medium">Personaliza con nombre y número</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end shrink-0">
                    <span className={`text-lg font-black font-headline ${estampado ? 'text-primary' : 'text-zinc-400'}`}>+$200</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${estampado ? 'text-primary' : 'text-zinc-300'}`}>{estampado ? 'Seleccionado ✓' : 'MXN'}</span>
                  </div>
                </button>
                {estampado && (
                  <div className="grid grid-cols-2 gap-4 bg-surface-container p-6 rounded-xl">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" maxLength={2} value={numeroEstampado}
                        onChange={e => setNumeroEstampado(e.target.value.replace(/\D/g, ''))}
                        placeholder="10"
                        className="w-full border-2 border-outline-variant rounded-lg px-4 py-3 text-sm font-black tracking-widest outline-none focus:border-primary placeholder:text-zinc-300 placeholder:font-normal" />
              ...
              {/* Preset multi: un toggle por jersey */}
              {isPresetMode && (
                <div className="bg-surface-container rounded-xl p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-black uppercase tracking-tight">Jersey Estampado</p>
                      <p className="text-xs text-on-surface-variant font-medium mt-0.5">Elige cuáles jerseys quieres estampar · +$200 por jersey</p>
                    </div>
                    {slotEstampadoCount > 0 && (
                      <span className="text-sm font-black text-primary">+${slotEstampadoCount * 200}</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    {jerseySlots.map((slot, i) => (
                      <div key={i} className="space-y-3">
                        <button onClick={() => updateSlot(i, 'estampado', !slot.estampado)}
                          className={`w-full flex items-center justify-between gap-3 px-4 py-3 border-2 rounded-xl transition-all text-left ${slot.estampado ? 'border-primary bg-emerald-50' : 'border-outline-variant hover:border-primary'}`}>
                          <div className="flex items-center gap-3">
                            <span className={`material-symbols-outlined text-lg ${slot.estampado ? 'text-primary' : 'text-zinc-400'}`}>style</span>
                            <span className="text-xs font-black uppercase tracking-tight">Jersey {i + 1}</span>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className={`text-xs font-black font-headline ${slot.estampado ? 'text-primary' : 'text-zinc-400'}`}>+$200</span>
                            {slot.estampado
                              ? <span className="text-[9px] font-black text-white bg-primary px-2 py-0.5 rounded-full uppercase">Con estampado ✓</span>
                              : <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wide">Sin estampado</span>
                            }
                          </div>
                        </button>
                        ...
                        {slot.estampado && (
                          <div className="grid grid-cols-2 gap-3 bg-white rounded-lg px-4 py-3 ml-4">
                            ...
                            <div className="space-y-1">
                              <label className="text-[9px] font-black uppercase tracking-widest text-on-surface-variant">Número <span className="text-zinc-400 normal-case tracking-normal font-normal">(2 dígitos)</span></label>
                              <input type="text" maxLength={2} value={slot.numeroEstampado}
                                onChange={e => updateSlot(i, 'numeroEstampado', e.target.value.replace(/\D/g, ''))}
                                placeholder="10"
                                className="w-full border-2 border-outline-variant rounded-lg px-3 py-2 text-xs font-black tracking-widest outline-none focus:border-primary placeholder:text-zinc-300 placeholder:font-normal" />
                              <p className="text-[9px] text-zinc-400">{slot.numeroEstampado.length}/2</p>
                            </div>
          ...
          <div className="sticky top-32 space-y-6">
            <div className="bg-white p-8 rounded-xl shadow-sm border border-outline-variant/10 space-y-8">
              ...
              <div className="space-y-4">
                ...
                {!isMultiJersey && (<>
                  ...
                  <div className="flex justify-between items-center py-3 border-b border-surface-container">
                    ...
                    <span className="text-sm font-bold uppercase">{TIPOS_SINGLE.find(t => t.id === selectedTipo)?.label ?? '—'}</span>
                  </div>
                </>)}
                  ...
                  {slotEstampadoCount > 0 && (
                    <div className="py-3 border-b border-surface-container space-y-1">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] font-bold uppercase px-2 py-1 bg-primary/10 text-primary rounded">+${slotEstampadoCount * 200}</span>
                      </div>
                      {jerseySlots.map((s, i) => s.estampado && (
                        <p key={i} className="text-[9px] font-black text-primary text-right">
                          J{i + 1}: {s.nombreEstampado || '—'} #{s.numeroEstampado || '—'}
                        </p>
                      ))}
                    </div>
                  )}
                    ...
                    {jerseySlots.map((slot, i) => (
                      <div key={i} className="text-xs">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`font-bold uppercase ${slot.tipo === 'retro' ? 'text-amber-600' : ''}`}>{slot.tipo} · {slot.talla}</span>
                        </div>
                        {slot.estampado && (
                ...
                {exclusiones.length > 0 && (
                  <div className="flex justify-between items-start py-3 border-b border-surface-container">
                    ...
                    <div className="text-right">{exclusiones.map(eq => <p key={eq} className="text-[10px] font-bold uppercase">{eq}</p>)}</div>
                  </div>
                )}
              ...
              <div className="pt-4">
                ...
                <button onClick={handleAgregarAlCarrito} disabled={added || !canAdd}
                  className={`w-full py-5 rounded-lg font-black uppercase tracking-[0.2em] text-sm transition-all flex items-center justify-center gap-3 ${
                    ...
                    : !canAdd ? 'bg-zinc-200 text-zinc-400 cursor-not-allowed'
...
export default function ConfigurarPage() {
  return <Suspense><ConfiguradorContent /></Suspense>
}
